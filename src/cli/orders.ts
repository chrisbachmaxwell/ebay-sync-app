  ...
  orders
    ...
    .action(async (opts: { since?: string; dryRun?: boolean; json?: boolean }) => {
      ...
      try {
        ...
        spinner.succeed(
          `Order sync complete: ${result.imported} imported, ${result.skipped} skipped, ${result.failed} failed`,
        );
        ...
        if (result.errors.length) {
          ...
          result.errors.forEach((e) =>
            logError(`  ${e.ebayOrderId}: ${e.error}`),
          );
        }
      } catch (err) {
        spinner.fail(
    ...
    .action(async (opts: { limit?: string; json?: boolean }) => {
      try {
        ...
        if (opts.json) {
          ...
          mappings.forEach((m) => {
            info(
              `  ${m.ebayOrderId} → ${m.shopifyOrderName ?? m.shopifyOrderId} (${m.status})`,
            );
          });
        }
      } catch (err) {
        logError(
          err instanceof Error ? err.message : 'Failed to list orders',
        );
        process.exitCode = 1;
      }
    });
