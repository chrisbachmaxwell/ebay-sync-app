...
const seedHelpContent = (sqlite: InstanceType<typeof Database>) => {
  ...
  const entries: [string, string, string][] = [
    [
      ...
      `Syncing products from Shopify to eBay is one of the core features of ProductBridge. Here's how to do it step by step:
...
You can monitor the pipeline on the **Pipeline** page, which shows all jobs and their current stages. If any step fails, the pipeline provides clear error messages so you can fix issues and retry. You can also manually trigger the pipeline for specific products using the auto-list button on the product detail page.`,
    ...
    [
      ...
      `ProductBridge uses PhotoRoom integration to automatically process and enhance product images before they're listed on eBay. Here's what happens:
...
Image processing requires a valid PhotoRoom API key configured in your environment settings. The system processes images asynchronously so it doesn't block the listing pipeline — products can continue through other stages while images are being processed.`,
    ...
    [
      ...
      `Order management in ProductBridge syncs eBay orders into Shopify for unified fulfillment. Here's how it works:
...
**Ask Questions**: Type any question about the app, your products, listings, or orders. The assistant understands the full capabilities of ProductBridge and can provide step-by-step guidance.
...
The chat assistant uses the capabilities registry to stay up-to-date with all available features. When new features are added to ProductBridge, the assistant automatically learns about them and can help you use them. You can access the chat from any page in the app — it maintains context across your conversation so you can have multi-step interactions.`,
    ...
    [
      ...
      `ProductBridge includes a built-in feature request system so you can suggest improvements and new features directly from the app.
...
**Stale Listings**: Listings that haven't been updated in a while may lose visibility on eBay. ProductBridge identifies stale listings (configurable threshold, default 30 days) and can automatically republish them to boost their search ranking. Go to Listings and look for the stale indicator, or use the chat assistant to ask "show stale listings."
...
**How It Works**: When a listing has been active for longer than the configured threshold (default: 14 days without a sale), ProductBridge can automatically reduce the price by a percentage (default: 10%). The original price is always preserved in the database so you can restore it later if needed.
...
**Manual Control**: You can also apply price drops manually from the Listings page or through the chat assistant. Ask "apply price drops" to trigger a manual run, or "drop prices on stale items" to target only items that meet the staleness criteria. All price drop actions are logged in the Analytics page so you have a full audit trail.`,
    ...
    [
      ...
      `ProductBridge supports exporting and importing field mappings so you can back up your configuration or transfer it between environments.
...
**Best Practices**: Export your mappings before making major changes so you have a backup. If you're setting up a new environment, export from your working environment and import into the new one. The export file is human-readable JSON, so you can also edit it manually if needed before importing.`,
    ...
    [
      ...
      `The capabilities registry is an internal system that tracks all features available in ProductBridge. It serves two important purposes:

**For the Chat Assistant**: The AI chat assistant uses the capabilities registry to know what features exist and how to help you use them. When a new feature is added to ProductBridge, it's registered in the capabilities system, and the chat assistant automatically learns about it. This means you can always ask the chat "what can you do?" and get an accurate, up-to-date answer.
...
The registry is designed to be self-documenting — as the app grows, the registry grows with it, ensuring that documentation, chat capabilities, and feature discovery all stay in sync automatically.`,
    ...
    [
      ...
      `ProductBridge provides powerful filtering and search capabilities to help you find and manage your products efficiently.
...
**Combining Filters**: You can combine search with status filters to narrow down results. For example, search for "lens" with status "active" to see all currently listed lens products. The URL updates with your filter parameters so you can bookmark specific views or share them.`,
    ...
    [
      ...
      `ProductBridge includes several safety guards to protect your data and prevent accidental damage:
...
**Rate Limiting**: All API endpoints are rate-limited to prevent accidental floods of requests. The eBay and Shopify APIs have their own rate limits, and ProductBridge respects them with automatic retry and backoff logic.
...
**Error Handling**: All sync operations include comprehensive error handling. If something fails, the error is logged with details, and the system continues processing remaining items rather than stopping entirely. Failed operations can be retried individually.`,
    ...
    [
      ...
      `Getting started with ProductBridge involves a few setup steps to connect your Shopify and eBay accounts and configure your preferences:

**Step 1 — Connect Shopify**: Go to **Settings** and enter your Shopify store credentials. ProductBridge needs your store URL and API access token to read product data and create orders. The connection status is shown on the Dashboard.

**Step 2 — Connect eBay**: In Settings, follow the eBay authentication flow. You'll be redirected to eBay to authorize ProductBridge to manage your listings. Once connected, the eBay token is stored securely and refreshes automatically.
...
The Dashboard provides a quick overview of your connections, product counts, and recent sync activity. Check it regularly to ensure everything is running smoothly. If you need help at any point, use the chat assistant or browse this FAQ.`,
    ...
    [
      ...
      `When sync operations encounter errors, ProductBridge provides detailed logging and diagnostics to help you identify and fix issues:
...
If errors persist, use the chat assistant to describe the problem. It can check logs, identify patterns, and suggest specific fixes based on the error details.`,
    ...
    [
      ...
      `ProductBridge currently supports eBay's fixed-price listing format (also known as "Buy It Now"), which is the most common format for professional sellers.

**Fixed-Price Listings**: All listings created through ProductBridge are fixed-price with optional Best Offer. This format is ideal for used camera gear because buyers can purchase immediately or make offers. You can configure Best Offer settings in your field mappings, including auto-accept and auto-decline thresholds.
...
**Automatic Updates**: When enabled, ProductBridge periodically checks your Shopify inventory levels and updates the corresponding eBay listings. If an item sells on Shopify, the eBay quantity is reduced. If you restock, the eBay quantity increases.
...
**Safety**: Inventory sync will never increase eBay quantity above the Shopify quantity. If there's a discrepancy, the system defaults to the lower number to prevent overselling. All inventory updates are logged in the sync history for auditability.`,
    ...
    [
      ...
      `There are several ways to get help with ProductBridge:
...
**Feature Requests**: Have an idea for improving ProductBridge? Submit a feature request at the **Feature Requests** page (Settings & Analytics > Feature Requests). Describe the feature you'd like and why it would be useful.
